...
async function sanitizeAndMark(page, rect, label) {
  await page.evaluate(({ rect, label }) => {
    ...
    nodes.forEach((node) => {
      ...
      value = value.replace(uuidRe, '[ID censurado]');
...
async function captureWebmailFallback(page, local) {
  ...
  await newPage.evaluate(() => {
    const target = [...document.querySelectorAll('button, input[type="button"], input[type="submit"], a, [role="button"]')]
      .find((el) => /Salvar e Continuar|Save and Continue/i.test([el.innerText, el.textContent, el.value, el.getAttribute('aria-label'), el.getAttribute('title')].filter(Boolean).join(' ')));
  ...
  await newPage.evaluate(() => {
    [...document.querySelectorAll('body *')].forEach((el) => {
      ...
      const style = getComputedStyle(el);
      const box = el.getBoundingClientRect();
      if (/Consent and Privacy|Salvar e Continuar|user activity tracking/i.test(text) && (style.position === 'fixed' || style.position === 'absolute' || el.getAttribute('role') === 'dialog' || (box.width > 500 && box.height > 300))) {
        el.remove();
      }
    });
    document.querySelectorAll('.modal-backdrop, .backdrop, [class*="backdrop"], [class*="modal"]').forEach((el) => {
      const text = (el.innerText || '').replace(/\s+/g, ' ').trim();
      if (!text || /Consent and Privacy|Salvar e Continuar|user activity tracking/i.test(text)) el.remove();
    });
