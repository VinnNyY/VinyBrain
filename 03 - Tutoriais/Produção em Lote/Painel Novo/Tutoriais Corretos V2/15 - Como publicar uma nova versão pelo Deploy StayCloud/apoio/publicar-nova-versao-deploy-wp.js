...
const canonicalSlug = 'publicar-nova-versao-staycloud';
const previousSlug = 'publicar-nova-versao-deploy-staycloud';
const title = 'PUBLICAR NOVA VERSÃO STAYCLOUD';
const focusKeyword = 'publicar nova versão StayCloud';
const seoTitle = 'Publicar nova versão StayCloud em 5 passos';
const metaDescription = 'Veja como publicar nova versão StayCloud pelo Deploy, executar o comando correto, acompanhar o status e confirmar a aplicação atualizada.';
const excerpt = 'Aprenda a publicar uma nova versão pelo Deploy StayCloud usando o projeto já conectado, o comando correto da CLI e a validação do status no painel.';
const socialTitle = 'Publicar nova versão StayCloud em 5 passos';
const socialDescription = 'Veja como atualizar um projeto já conectado ao Deploy StayCloud, executar o comando de publicação e confirmar a nova versão no ar.';
const painelNovoCategoryId = 17;
const tagNames = ['Deploy', 'Atualização', 'Aplicações', 'Nova Versão', 'Desenvolvimento', 'StayCloud'];

const images = [
  { file: '01-projeto-publicado-sanitizado.png', alt: 'Projeto publicado para publicar nova versão StayCloud', title: 'Projeto publicado no Cloud' },
  { file: '02-versao-atual-sanitizado.png', alt: 'Versão atual antes de publicar nova versão StayCloud', title: 'Versão atual publicada' },
  { file: '03-alteracao-projeto-sanitizado.png', alt: 'Alteração local antes de publicar nova versão StayCloud', title: 'Alteração local do projeto' },
  { file: '04-nova-publicacao-sanitizado.png', alt: 'Comando para publicar nova versão StayCloud', title: 'Comando de nova publicação' },
  { file: '05-processamento-sanitizado.png', alt: 'Status após publicar nova versão StayCloud', title: 'Status após novo deploy' },
  { file: '06-conclusao-sanitizado.png', alt: 'Projeto pronto após publicar nova versão StayCloud', title: 'Projeto pronto no painel' },
  { file: '07-nova-versao-publicada-sanitizado.png', alt: 'Nova versão publicada pelo Deploy StayCloud', title: 'Nova versão publicada' },
];
...
async function main() {
  ...
  if (slugMatches.some((doc) => doc.status === 'publish')) throw new Error(`Slug já publicado: ${canonicalSlug}`);
  ...
  const payload = { title, content: publicHtml, excerpt, slug: canonicalSlug, status: 'draft', doc_category: [painelNovoCategoryId], doc_tag: tagIds, featured_media: uploaded[0] && uploaded[0].id };
  const draft = existingDraft
    ? await wpFetch(page, `/wp-json/wp/v2/docs/${existingDraft.id}`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) })
