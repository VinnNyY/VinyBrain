...
const values = {
  ...
  focus: 'busca do Painel Novo StayCloud',
  seoTitle: 'Busca do Painel Novo StayCloud: use em 3 passos',
  description: 'Aprenda a usar a busca do Painel Novo StayCloud para localizar sites, domínios, faturas e chamados com rapidez, segurança e organização no painel.',
...
async function openRankMath(page) {
  await page.evaluate(() => {
    const item = Array.from(document.querySelectorAll('button,[role="button"]')).find((el) => {
      const label = (el.innerText || el.textContent || el.getAttribute('aria-label') || '').trim();
      return (el.getAttribute('aria-label') || '').includes('Rank Math') || /\b\d+\s*\/\s*100\b/.test(label);
    });
    if (item) item.click();
  });
  await wait(2200);
...
async function collectState(page) {
  return page.evaluate((values) => {
    ...
    return {
      ...
      lines: text.split('\n').filter((line) => /Rank Math|Palavra|Foco|Fragmento|Snippet|SEO|Social|Busca|Painel Novo|100|Descrição|Titulo|Título|Slug|erro/i.test(line)).slice(0, 120),
    };
  }, values);
}
